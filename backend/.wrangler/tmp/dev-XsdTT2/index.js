...
// .wrangler/tmp/bundle-Crlvko/checked-fetch.js
...
// .wrangler/tmp/bundle-Crlvko/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-Crlvko/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var GroqService = class {
  ...
  async callGroqApi({ messages, model = DEFAULT_MODEL, temperature = DEFAULT_TEMPERATURE, maxTokens = DEFAULT_MAX_TOKENS, image }) {
    try {
      ...
      if (model in MODEL_MAPPING) {
        ...
        console.log(`\u{1F504} \u5207\u63DB\u5230\u6A21\u578B: ${modelDisplayName} (ID: ${model})`);
        console.log(`\u{1F4DD} \u6A21\u578B\u53C3\u6578: \u6EAB\u5EA6=${actualTemperature}, \u6700\u5927Tokens=${actualMaxTokens}`);
  ...
  async enhancedChat(options, limit = 3, threshold = 0.3) {
    try {
      ...
      console.log(`\u{1F50D} RAG: \u958B\u59CB\u8655\u7406\u7528\u6236\u67E5\u8A62: "${query.substring(0, 50)}${query.length > 50 ? "..." : ""}"`);
      ...
      console.log(`\u{1F50D} RAG: \u958B\u59CB\u5728\u5411\u91CF\u8CC7\u6599\u5EAB\u4E2D\u641C\u5C0B\u76F8\u95DC\u8CC7\u8A0A (\u9650\u5236: ${limit}, \u95BE\u503C: ${threshold})`);
      const faqs = await pinecone.searchFaqs(query, limit, threshold);
      console.log(`\u{1F50D} RAG: \u627E\u5230 ${faqs.length} \u500B\u76F8\u95DC\u7B54\u6848`);
      let finalOptions = { ...options };
      if (faqs.length > 0) {
        console.log(`\u{1F504} RAG: \u5275\u5EFA\u589E\u5F37\u7684\u7CFB\u7D71\u63D0\u793A\u8A5E\uFF0C\u6574\u5408 ${faqs.length} \u500B\u76F8\u95DC\u77E5\u8B58\u9EDE`);
        faqs.forEach((faq, index) => {
          console.log(`\u{1F504} RAG: FAQ #${index + 1}: "${faq.question.substring(0, 30)}..." (\u76F8\u4F3C\u5EA6: ${faq.score.toFixed(2)})`);
        });
        ...
        console.log(`\u{1F916} RAG: \u4F7F\u7528\u589E\u5F37\u63D0\u793A\u8A5E\u8ABF\u7528 Groq API`);
        return this.callGroqApiWithSystemPrompt(finalOptions, enhancedSystemPrompt);
      } else {
        console.log(`\u{1F916} RAG: \u672A\u627E\u5230\u76F8\u95DC\u77E5\u8B58\uFF0C\u4F7F\u7528\u57FA\u672C\u7CFB\u7D71\u63D0\u793A\u8A5E`);
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      console.log(`\u{1F7E2} [${requestId}] \u8ACB\u6C42\u5B8C\u6210: ${newResponse.status}, \u8017\u6642 ${processingTime}ms`);
      return newResponse;
    } catch (error) {
      const processingTime = Date.now() - startTime;
