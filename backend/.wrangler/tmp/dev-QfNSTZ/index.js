...
// .wrangler/tmp/bundle-2mGDFB/checked-fetch.js
...
// .wrangler/tmp/bundle-2mGDFB/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
init_process();
init_buffer();

// .wrangler/tmp/bundle-2mGDFB/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
init_process();
init_buffer();
...
var httpAuthSchemeMiddleware = /* @__PURE__ */ __name((config2, mwOptions) => (next, context) => async (args) => {
...
var httpSigningMiddleware = /* @__PURE__ */ __name((config2) => (next, context) => async (args) => {
...
function modeOf(chunk) {
  if (typeof Buffer2 !== "undefined" && chunk instanceof Buffer2) {
    return 2;
  }
...
var resolveAwsSdkSigV4Config = /* @__PURE__ */ __name((config2) => {
  ...
  if (!credentialsProvider) {
    if (config2.credentialDefaultProvider) {
      credentialsProvider = normalizeProvider2(config2.credentialDefaultProvider(Object.assign({}, config2, {
  ...
  const boundCredentialsProvider = /* @__PURE__ */ __name(async () => credentialsProvider({ callerClientConfig: config2 }), "boundCredentialsProvider");
...
var constructStack = /* @__PURE__ */ __name(() => {
  ...
  const stack = {
    ...
    concat: (from2) => {
      ...
      cloned.use(from2);
      cloned.identifyOnResolve(identifyOnResolve || cloned.identifyOnResolve() || (from2.identifyOnResolve?.() ?? false));
      return cloned;
    },
...
// node_modules/@aws-crypto/util/build/module/convertToBuffer.js
var fromUtf83 = typeof Buffer2 !== "undefined" && Buffer2.from ? function(input) {
...
var flexibleChecksumsMiddleware = /* @__PURE__ */ __name((config2, middlewareConfig) => (next, context) => async (args) => {
  if (!HttpRequest.isInstance(args.request)) {
    return next(args);
  }
...
var flexibleChecksumsInputMiddleware = /* @__PURE__ */ __name((config2, middlewareConfig) => (next, context) => async (args) => {
...
var flexibleChecksumsResponseMiddleware = /* @__PURE__ */ __name((config2, middlewareConfig) => (next, context) => async (args) => {
  if (!HttpRequest.isInstance(args.request)) {
    return next(args);
  }
  const input = args.input;
...
var s3ExpressHttpSigningMiddleware = /* @__PURE__ */ __name((config2) => (next, context) => async (args) => {
  if (!HttpRequest.isInstance(args.request)) {
    return next(args);
  }
  const smithyContext = getSmithyContext(context);
...
var throw200ExceptionsMiddleware = /* @__PURE__ */ __name((config2) => (next, context) => async (args) => {
  const result = await next(args);
...
// node_modules/@aws-sdk/client-s3/dist-es/auth/httpAuthSchemeProvider.js
var createEndpointRuleSetHttpAuthSchemeParametersProvider = /* @__PURE__ */ __name((defaultHttpAuthSchemeParametersProvider) => async (config2, context, input) => {
  if (!input) {
...
var _defaultS3HttpAuthSchemeParametersProvider = /* @__PURE__ */ __name(async (config2, context, input) => {
  return {
...
async function blobReader(blob, onChunk, chunkSize = 1024 * 1024) {
  ...
  while (totalBytesRead < size) {
    const slice2 = blob.slice(totalBytesRead, Math.min(size, totalBytesRead + chunkSize));
    onChunk(new Uint8Array(await slice2.arrayBuffer()));
    totalBytesRead += slice2.size;
  }
}
...
// src/handlers/upload.ts
var S3_CLIENT = null;
function getS3Client(env2) {
  if (S3_CLIENT)
    return S3_CLIENT;
  S3_CLIENT = new S3Client({
    region: "auto",
    endpoint: env2.R2_API_ENDPOINT,
    credentials: {
...
async function handleUpload(request, env2) {
  ...
  try {
    ...
    const s3Client = getS3Client(env2);
    ...
    const fileUrl = `https://${env2.R2_ENDPOINT}/${path}`;
    return new Response(JSON.stringify({
      success: true,
      url: fileUrl
    }), {
...
async function handleChat(request, env2) {
  ...
  try {
    ...
    const groqResponse = await callGroqApi(data, env2);
    return new Response(JSON.stringify({
      success: true,
      data: groqResponse
    }), {
...
async function callGroqApi({ messages, model = "llama-3.1-8b-instant", temperature = 0.7, maxTokens = 2048 }, env2) {
  try {
...
var PineconeClient = class {
   ...
   * @returns FAQ 搜尋結果
   */
  async searchFaqs(query, limit = 5, threshold = 0.75) {
    try {
      ...
      const data = await response.json();
      const results = data.matches.filter((match) => match.score >= threshold).map((match) => ({
        id: match.id,
...
__name(PineconeClient, "PineconeClient");

// src/handlers/faq.ts
async function handleFaq(request, env2) {
  const headers = { ...corsHeaders, "Content-Type": "application/json" };
  try {
    if (request.method !== "POST") {
      return new Response(JSON.stringify({
    ...
    const pinecone = new PineconeClient(
      env2.PINECONE_API_KEY,
      env2.PINECONE_ENVIRONMENT,
      env2.PINECONE_INDEX
    );
  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/faq") {
        return handleFaq(request, env2);
      }
      if (url.pathname === "/api/upload") {
        return handleUpload(request, env2);
      }
