...
// .wrangler/tmp/bundle-iPsVIb/checked-fetch.js
...
// .wrangler/tmp/bundle-iPsVIb/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-iPsVIb/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// node_modules/@aws-sdk/middleware-flexible-checksums/dist-es/validateChecksumFromResponse.js
var validateChecksumFromResponse = /* @__PURE__ */ __name(async (response, { config, responseAlgorithms, logger: logger4 }) => {
    ...
    if (checksumFromResponse) {
      ...
      try {
        ...
        if (algorithm === ChecksumAlgorithm.CRC64NVME) {
          logger4?.warn(`Skipping ${ChecksumAlgorithm.CRC64NVME} checksum validation: ${error.message}`);
          continue;
        }
        throw error;
      }
...
init_modules_watch_stub();
var LOG_STYLES = {
  [0 /* DEBUG */]: { emoji: "\u{1F50D}", color: "\x1B[36m" },
  // 青色
  [1 /* INFO */]: { emoji: "\u{1F535}", color: "\x1B[34m" },
  // 藍色
  [2 /* WARN */]: { emoji: "\u26A0\uFE0F", color: "\x1B[33m" },
  // 黃色
  [3 /* ERROR */]: { emoji: "\u{1F534}", color: "\x1B[31m" },
  // 紅色
  [4 /* FATAL */]: { emoji: "\u{1F4A5}", color: "\x1B[35m" }
  // 紫色
};
var LOG_LEVEL_NAMES = {
  [0 /* DEBUG */]: "DEBUG",
...
function formatObject(obj) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    const isKeywordSensitive = sensitiveFields.some((field) => key.toLowerCase().includes(field.toLowerCase()));
...
var Logger = class _Logger {
   ...
   * @returns 格式化後的日誌對象
   */
  formatLogMessage(level, message, requestId, details) {
    ...
    const processedDetails = details ? formatObject(details) : void 0;
    return {
   ...
   */
  log(level, message, detailsOrRequestId, extraDetails) {
   ...
   * @param extraDetails 額外的詳細信息，只在 detailsOrRequestId 為請求ID 時使用
   */
  debug(message, detailsOrRequestId, extraDetails) {
    this.log(0 /* DEBUG */, message, detailsOrRequestId, extraDetails);
  }
  /**
   * 記錄信息級別日誌
   * @param message 日誌消息
   ...
   * @param extraDetails 額外的詳細信息，只在 detailsOrRequestId 為請求ID 時使用
   */
  info(message, detailsOrRequestId, extraDetails) {
    this.log(1 /* INFO */, message, detailsOrRequestId, extraDetails);
  }
  /**
   * 記錄警告級別日誌
   * @param message 日誌消息
   ...
   * @param extraDetails 額外的詳細信息，只在 detailsOrRequestId 為請求ID 時使用
   */
  warn(message, detailsOrRequestId, extraDetails) {
    this.log(2 /* WARN */, message, detailsOrRequestId, extraDetails);
  }
  /**
   * 記錄錯誤級別日誌
   * @param message 日誌消息
   ...
   * @param extraDetails 額外的詳細信息，只在 detailsOrRequestId 為請求ID 時使用
   */
  error(message, detailsOrRequestId, extraDetails) {
    this.log(3 /* ERROR */, message, detailsOrRequestId, extraDetails);
  }
  /**
   * 記錄致命錯誤級別日誌
   * @param message 日誌消息
   ...
   * @param extraDetails 額外的詳細信息，只在 detailsOrRequestId 為請求ID 時使用
   */
  fatal(message, detailsOrRequestId, extraDetails) {
    this.log(4 /* FATAL */, message, detailsOrRequestId, extraDetails);
  }
  /**
   * 記錄錯誤對象
   * @param error 錯誤對象
   ...
   */
  logError(error, requestId, message) {
   ...
   * @param config 覆蓋配置
   */
  createSubLogger(subContext, config = {}) {
    const contextName = `${this.context}:${subContext}`;
    return new _Logger(contextName, { ...this.config, ...config });
  }
  /**
   * 創建一個請求專屬的日誌記錄器
   * @param requestId 請求ID
...
var RequestLogger = class {
   ...
   */
  logRequestEnd(response, processingTime) {
...
var logger2 = new Logger("EchoMind");
var apiLogger = logger2.createSubLogger("API");
var chatLogger = logger2.createSubLogger("Chat");
var faqLogger = logger2.createSubLogger("FAQ");
var vectorLogger = logger2.createSubLogger("Vector");
var uploadLogger = logger2.createSubLogger("Upload");
var envLogger = logger2.createSubLogger("Env");
...
function handleError(error, request, requestId) {
  logger2.error("\u8655\u7406\u932F\u8AA4", requestId, error);
  let message = "\u4F3A\u670D\u5668\u5167\u90E8\u932F\u8AA4";
  let status = 500;
  let details = void 0;
  if (error instanceof Error) {
    message = error.message;
    logger2.error("\u932F\u8AA4\u8A73\u60C5", requestId, {
      name: error.name,
      message: error.message,
      stack: error.stack
    });
...
async function handleUpload(request, env) {
  ...
  const logger4 = uploadLogger.forRequest(requestId);
  try {
    logger4.info("\u958B\u59CB\u8655\u7406\u6A94\u6848\u4E0A\u50B3\u8ACB\u6C42");
...
var PineconeClient = class {
   ...
   */
  constructor(apiKey, environment, indexName, env, fullApiUrl, cacheConfig) {
    if (!apiKey) {
   ...
   * @returns 調整後的閾值
   */
  calculateDynamicThreshold(query, baseThreshold) {
  ...
  calibrateScore(score) {
    const scaleFactor = 8;
    const midPoint = 0.5;
    const calibrated = 1 / (1 + Math.exp(-scaleFactor * (score - midPoint)));
    const minOutput = 0.1;
   ...
   * @returns 整合後的結果
   */
  integrateSimilarFaqs(results) {
    if (results.length <= 1) {
      return results;
    }
    const INTEGRATION_THRESHOLD = 0.65;
    const MAX_RESULTS_TO_INTEGRATE = 5;
    const integratedIds = /* @__PURE__ */ new Set();
    const integratedResults = [];
    const sortedResults = [...results].sort((a2, b2) => b2.score - a2.score);
    for (let i2 = 0; i2 < sortedResults.length; i2++) {
      const currentFaq = sortedResults[i2];
      ...
      for (let j2 = i2 + 1; j2 < sortedResults.length; j2++) {
        if (integratedCount >= MAX_RESULTS_TO_INTEGRATE) {
          break;
        }
        const otherFaq = sortedResults[j2];
        ...
        if (currentFaq.category && otherFaq.category && currentFaq.category === otherFaq.category) {
          categoryMatch = true;
        }
  ...
  calculateQuestionSimilarity(question1, question2) {
    const words1 = question1.toLowerCase().split(/\s+/).filter((w2) => w2.length > 1);
    const words2 = question2.toLowerCase().split(/\s+/).filter((w2) => w2.length > 1);
    ...
    const charSimilarity = chars1.length + chars2.length > 0 ? 2 * commonChars.length / (chars1.length + chars2.length) : 0;
   ...
   * @returns 組合後的答案
   */
  combineAnswers(primaryAnswer, secondaryAnswer) {
    ...
    const primarySentences = primaryAnswer.split(/[.，,.!！？?]\s*/).filter((s2) => s2.trim());
    const secondarySentences = secondaryAnswer.split(/[.，,.!！？?]\s*/).filter((s2) => s2.trim());
    const uniqueSentences = new Set(primarySentences);
    for (const sentence of secondarySentences) {
      let isDuplicate = false;
      for (const existingSentence of uniqueSentences) {
        if (existingSentence.includes(sentence) || sentence.includes(existingSentence) || this.calculateStringSimilarity(existingSentence, sentence) > 0.7) {
          isDuplicate = true;
          break;
        }
      }
  ...
  calculateStringSimilarity(str1, str2) {
    ...
    for (let i2 = 0; i2 <= len1; i2++) dp[i2][0] = i2;
    for (let j2 = 0; j2 <= len2; j2++) dp[0][j2] = j2;
    for (let i2 = 1; i2 <= len1; i2++) {
      for (let j2 = 1; j2 <= len2; j2++) {
        const cost = s1[i2 - 1] === s2[j2 - 1] ? 0 : 1;
        dp[i2][j2] = Math.min(
          dp[i2 - 1][j2] + 1,
          // 刪除
          dp[i2][j2 - 1] + 1,
  ...
  async searchFaqs(query, limit = 5, threshold = 0.1) {
    ...
    const performSearch = /* @__PURE__ */ __name(async () => {
      ...
      if (data.matches && data.matches.length > 0) {
        const validMatches = data.matches.filter((match) => match.score >= dynamicThreshold);
        ...
        for (const match of validMatches) {
          ...
          const importance = match.metadata.importance || 1;
          const queryLower = query.toLowerCase();
          const queryWords = queryLower.split(/\s+/).filter((w2) => w2.length > 1);
          ...
          if (questionLower === queryLower) {
            ...
            textMatchScore = wordMatchRatio * 0.6;
          }
          ...
          const weightedScore = (textMatchScore * textMatchWeight + semanticScore * semanticWeight + importance * importanceWeight) * tagBoost;
        ...
        mergedResults.sort((a2, b2) => b2.score - a2.score);
...
function createEnhancedSystemPrompt(faqs) {
...
__name(createEnhancedSystemPrompt, "createEnhancedSystemPrompt");
var GroqService = class {
  static {
    __name(this, "GroqService");
  }
  /**
   * 環境變數
   */
   ...
   */
  constructor(env, cacheConfig) {
    this.env = env;
    this.cacheConfig = { ...DEFAULT_CACHE_CONFIG2, ...cacheConfig };
    this.responseCache = new MemoryCache({
      ttl: this.cacheConfig.ttl,
      maxSize: this.cacheConfig.maxSize
    });
    chatLogger.info("\u521D\u59CB\u5316 Groq \u670D\u52D9\u548C\u5FEB\u53D6", {
      cacheEnabled: this.cacheConfig.enabled,
      cacheTtl: this.cacheConfig.ttl / 1e3 / 60 + "\u5206\u9418",
  ...
  async callGroqApi({ messages, model = DEFAULT_MODEL, temperature = DEFAULT_TEMPERATURE, maxTokens = DEFAULT_MAX_TOKENS, image }) {
    if (this.cacheConfig.enabled && !image) {
      const cacheKey = createCacheKey(
        ...
        temperature,
        maxTokens,
        messages.map((m2) => `${m2.role}:${m2.content.substring(0, 100)}`)
      );
    ...
    const callApi = /* @__PURE__ */ __name(async () => {
      ...
      if (model in MODEL_MAPPING) {
        ...
        console.log(`\u{1F504} \u4F7F\u7528\u76F4\u63A5\u6307\u5B9A\u7684\u6A21\u578B: ${model}`);
      } else {
      ...
      console.log(`\u{1F310} \u767C\u9001\u8ACB\u6C42\u5230 Groq API (\u6A21\u578B: ${modelDisplayName})...`);
      const response = await fetch(url, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async enhancedChat(options, limit = 3, threshold = 0.3) {
    if (this.cacheConfig.enabled && this.cacheConfig.cacheRagResults && !options.image) {
      const cacheKey = createCacheKey(
        ...
        limit,
        threshold,
        options.messages.map((m2) => `${m2.role}:${m2.content.substring(0, 100)}`)
      );
      const cachedResponse = this.responseCache.get(cacheKey);
      if (cachedResponse) {
   ...
   * @returns Groq API 回應
   */
  async callGroqApiWithSystemPrompt(options, systemPrompt) {
...
// src/handlers/chat.ts
var DEFAULT_MODEL2 = getModelConfig("default").name;
var DEFAULT_TEMPERATURE2 = getModelConfig("default").temperature;
var DEFAULT_MAX_TOKENS2 = getModelConfig("default").maxTokens;
async function handleChat(request, env) {
  const requestId = crypto.randomUUID();
  const logger4 = chatLogger.forRequest(requestId);
  logger4.info("\u6536\u5230\u804A\u5929\u8ACB\u6C42", {
    url: request.url,
    method: request.method,
    origin: request.headers.get("Origin"),
  ...
  try {
    ...
    console.log(`\u{1F4DD} [${requestId}] \u8ACB\u6C42\u5167\u5BB9\u6458\u8981:`, {
      ...
      requestedModel: data.model || DEFAULT_MODEL2,
      temperature: data.temperature || DEFAULT_TEMPERATURE2,
      maxTokens: data.maxTokens || DEFAULT_MAX_TOKENS2,
    ...
    console.log(`\u{1F504} [${requestId}] \u958B\u59CB\u555F\u7528 RAG \u589E\u5F37\u804A\u5929...`);
    try {
...
async function handleFaq(request, env) {
  const requestId = crypto.randomUUID();
  const logger4 = faqLogger.forRequest(requestId);
  logger4.info("\u958B\u59CB\u8655\u7406 FAQ \u67E5\u8A62\u8ACB\u6C42");
  try {
    if (request.method !== "POST") {
      return createErrorResponse("\u65B9\u6CD5\u4E0D\u5141\u8A31", 405, request);
    }
...
init_checked_fetch();
init_modules_watch_stub();
function createResponse(data, status = 200, request, requestId) {
  const isSuccess = status >= 200 && status < 300;
  const body = {
    success: isSuccess,
    ...isSuccess ? { data } : { error: typeof data === "string" ? { message: data } : data },
    ...requestId ? { requestId } : {}
  };
...
function createRequestHandler(handler, options = {}) {
  const {
    requireAuth = false,
    logRequest = true,
    logResponse = true
  ...
  return async (request, env) => {
    ...
    try {
      ...
      const errorStatus = error instanceof Error && "statusCode" in error ? error.statusCode : 500;
...
// src/handlers/cache.ts
var clearAllCaches = createRequestHandler(
  async (request, env) => {
...
var getCacheStats = createRequestHandler(
  async (request, env) => {
    const globalCacheSize = globalCache.size();
    ...
    const stats = {
      globalCache: {
        ...
        keys: globalCacheKeys.slice(0, 20)
        // 只顯示前 20 個鍵以防止過大回應
      },
